// ACE Forged Test — 12 attack vectors on realistic sizes.
// Base game uses ONLY shared gameLogic functions with seeded RNG (matching frontend).
// Each attack round modifies one aspect and verifies rejection.

import { createEmptyGrid, placeMines, revealCellLogic, checkWin } from '../../shared/gameLogic';
...
const SIZES = [
  { rows: 8, cols: 8, mines: 10 }, { rows: 9, cols: 9, mines: 14 },
  { rows: 9, cols: 9, mines: 18 }, { rows: 10, cols: 10, mines: 18 },
  { rows: 12, cols: 12, mines: 28 }, { rows: 16, cols: 16, mines: 40 },
  { rows: 8, cols: 16, mines: 22 }, { rows: 16, cols: 8, mines: 22 },
  { rows: 10, cols: 10, mines: 22 }, { rows: 12, cols: 12, mines: 32 },
  { rows: 16, cols: 16, mines: 56 }, { rows: 8, cols: 8, mines: 14 },
];
...
interface ForgeResult {
  round: number; attackType: string; gridLabel: string;
  accountId: string; valid: boolean; reason: string | null; reward: string | null;
  passed: boolean; timeMs: number;
}

// ═══════════════════════════════════════════════════════════
// Builder — same shared functions + seeded RNG as frontend
// ═══════════════════════════════════════════════════════════
...
function buildBaseGame(rows: number, cols: number, mines: number, mineSeed: string): GameSubmission {
  ...
  let board: any[][] = createEmptyGrid(rows, cols);
    ...
    if (!acted) {
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          board = cr.grid;
          if (cr.exploded) board[r][c].status = 'flagged';
          acted = true; break;
        }
        if (acted) break;
      }
  ...
  return {
    ...
    grid: { rows, cols, mines }, mine_seed: mineSeed,
    actions, prayers_used: prayers, total_time_ms: ts,
  };
}
...
async function main() {
  ...
  for (let i = 0; i < ATTACKS.length; i++) {
    ...
    try {
      ...
      r.passed = (!r.valid && !r.reward); r.timeMs = Date.now() - start;
