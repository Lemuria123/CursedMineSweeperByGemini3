...
async function main() {
  ...
  db.run('DELETE FROM records WHERE rows=25 AND cols=16');
  ...
  for (let i = 0; i < T.length; i++)
    db.run('INSERT INTO records (account_id,rows,cols,mines,time_ms,game_data,validated,submitted_at,prayers_used) VALUES (?,?,?,?,?,\'x\',1,?,0)',
      [T[i][0], 25, 16, MN(25, 16), T[i][1], N - i * 1000]);

  db.run(`DELETE FROM records WHERE account_id='${YOUR}' AND rows=25 AND cols=16`);
  db.run('INSERT INTO records (account_id,rows,cols,mines,time_ms,game_data,validated,submitted_at,prayers_used) VALUES (?,?,?,?,?,\'x\',1,?,0)',
    [YOUR, 25, 16, MN(25, 16), 2500, N - 100000]);

  console.log('25x16:');
  const r = db.exec('SELECT a.nickname, r.time_ms FROM records r LEFT JOIN accounts a ON r.account_id=a.id WHERE r.rows=25 AND r.cols=16 AND r.validated=1 ORDER BY r.time_ms ASC');
